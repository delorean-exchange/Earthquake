...
contract RewardBalances {
    ...
    function balanceOf(address account) external view returns (uint256) {
        ...
        for (uint256 i; i < stakingRewardsContracts.length; i++) {
            // i initialize to 0
            balance = balance.add(
        ...
        public
    {
        ...
        require(
            _stakingRewardsContract != address(0),
            "RewardBalances: ZERO_ADDRESS"
        );
    ...
    function removeStakingContractAddress(uint8 _index) external {
        require(msg.sender == owner, "RewardBalances: FORBIDDEN");
        require(
            _index <= stakingRewardsContracts.length - 1,
            "RewardBalances: OUT_OF_BOUNDS"
        );
